    ...
    <div className="input-field col s12">
      <select id={ this.properties.formDistinguisher+"-input-"+this.properties.distinguisher }
       ...
       className="form-field">
        <option value="" selected={ this.properties.value == null || this.properties.value == "" }>Choose an option</option>
        { Object.keys(this.properties.options).map(key => (
          <option value={ key } selected={ this.properties.value == key }>{ this.properties.options[key] }</option>
        )) }
      </select>
