<!DOCTYPE html>
<html>
	<head>
		<title>
			Catalyst 
		</title>

		<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js" ></script>
		<script src="https://cdnjs.cloudflare.com/ajax/libs/materialize/0.100.2/js/materialize.js" ></script>

		<link href="https://cdnjs.cloudflare.com/ajax/libs/materialize/0.100.2/css/materialize.css" rel="stylesheet" />
		<link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet" />
		<link href="css.css" rel="stylesheet" />

		<meta content="text/html; charset=utf-8" http-equiv="Content-Type" />
		<meta content="width=device-width, initial-scale=1, maximum-scale=1.0" name="viewport" />
		<meta charset="utf-8" />

		<meta name="description" content="Catalyst serves to facilitate the process of commissioning through a simple, unified, and mobile-friendly way for artists to easily list their prices, receive and track commissions, and much more."/>
		<meta name="keywords" content="Catalyst"/>
		<meta name="subject" content="Art, furry, commissions, catalyst"/>
		<meta name="copyright" content="Catalyst"/>
		<meta name="language" content="EN"/>
		<meta name="robots" content="index,follow"/>
		<meta name="Classification" content="Business"/>
		<meta name="author" content="Fauxil Fox, fauxil@catalystapp.co"/>
		<meta name="designer" content="Fauxil Fox"/>
		<meta name="reply-to" content="fauxil@catalystapp.co"/>

		<!-- Apple -->
		<meta name="apple-mobile-web-app-title" content="Catalyst"/>
		<meta name="apple-mobile-web-app-capable" content="yes"/>
		<meta name="apple-touch-fullscreen" content="yes"/>
		<meta name="apple-mobile-web-app-status-bar-style" content="black"/>
		<meta name="format-detection" content="telephone=no"/>
		<link href="https://catalystapp.co/img/logo_square.png" rel="apple-touch-icon" type="image/png"/>
		<link href="https://catalystapp.co/img/logo_square.png" rel="apple-touch-icon-precomposed" type="image/png"/>
		<link href="https://catalystapp.co/img/logo_square.png" rel="apple-touch-icon" type="image/png"/>
		<link href="https://catalystapp.co/img/logo_square.png" rel="apple-touch-icon-precomposed" type="image/png"/>
		<link rel="mask-icon" href="https://catalystapp.co/img/logo_square.png" color="black"/>

		<!-- IE -->
		<meta name="msapplication-tooltip" content="Catalyst - Facilitating Commissions"/>
		<meta http-equiv="Page-Enter" content="RevealTrans(Duration=2.0,Transition=2)"/>
		<meta http-equiv="Page-Exit" content="RevealTrans(Duration=3.0,Transition=12)"/>
		<meta name="mssmarttagspreventparsing" content="true"/>
		<meta content="IE=edge,chrome=1" http-equiv="X-UA-Compatible"/>
		<meta name="msapplication-starturl" content="https://catalystapp.co/"/>
		<meta name="msapplication-window" content="width=800;height=600"/>
		<meta name="msapplication-navbutton-color" content="green"/>
		<meta name="application-name" content="Catalyst"/>
		<meta http-equiv="cleartype" content="on"/>

		<!-- win 8+ -->
		<meta name="application-name" content="Catalyst"/>
		<meta name="msapplication-TileColor" content="#1b5e20"/>
		<meta name="msapplication-square70x70logo" content="img/logo_square.png"/>

		<!-- opengraph -->
		<meta property="og:title" content="Catalyst"/>
		<meta property="og:type" content="business.business"/>
		<meta property="og:url" content="https://catalystapp.co/"/>
		<meta property="og:image" content="https://catalystapp.co/img/logo_big_white.png"/>		
		<meta property="og:description" content="Catalyst serves to facilitate the process of commissioning through a simple, unified, and mobile-friendly way for artists to easily list their prices, receive and track commissions, and much more."/>
		<meta property="og:site_name" content="Catalyst"/>
		<meta property="og:locale" content="en_US"/>

		<!-- twitter -->
		<meta name="twitter:card" content="summary" />
		<meta name="twitter:site" content="https://catalystapp.co/" />
		<meta name="twitter:title" content="Catalyst" />
		<meta name="twitter:description" content="Catalyst serves to facilitate the process of commissioning through a simple, unified, and mobile-friendly way for artists to easily list their prices, receive and track commissions, and much more." />
		<meta name="twitter:image" content="https://catalystapp.co/img/logo_big_white.png" />

		<!-- link tags -->
		<link rel='shortcut icon' type='image/png' href='https://catalystapp.co/img/logo_square.png'/>
		<link rel='fluid-icon' type='image/png' href='https://catalystapp.co/img/logo_square.png'/>
		<link rel="canonical" href="https://catalystapp.co/"/>
		<link rel='publisher' href="https://plus.google.com/102762464787584663279/"/>
		<link rel="image_src" href="https://catalystapp.co/img/logo_square.png" type="image/png"/>
	</head>
	<body>
		<header>
			<div class="navbar-fixed">
				<nav class="green darken-4">
					<div class="nav-wrapper">
						<a class="brand-logo" href="">
							Catalyst 
						</a>
						<a class="button-collapse" data-activates="mobile-menu" href="#">
							<i class="material-icons">
								menu
							</i>
						</a>
						<ul class="right hide-on-med-and-down">
							<li>
								<a href="#about">
									About 
								</a>
							</li>
							<li>
								<a href="#roadmap">
									Roadmap 
								</a>
							</li>
							<li>
								<a href="#track">
									Track Development 
								</a>
							</li>
							<li>
								<a href="#involve">
									Get Involved 
								</a>
							</li>
							<li>
								<a href="#email">
									Email List 
								</a>
							</li>
						</ul>
					</div>
				</nav>
			</div>
			<ul class="side-nav" id="mobile-menu">
				<li>
					<a href="#about">
						About 
					</a>
				</li>
				<li>
					<a href="#roadmap">
						Roadmap 
					</a>
				</li>
				<li>
					<a href="#track">
						Track Development 
					</a>
				</li>
				<li>
					<a href="#involve">
						Get Involved 
					</a>
				</li>
				<li>
					<a href="#email">
						Email List 
					</a>
				</li>
			</ul>
		</header>
 
		<div class="container">
			<div class="section">
				<h1 class="center header hide-on-small-only">
					Catalyst
				</h1>
				<h2 class="center header hide-on-med-and-up">
					Catalyst
				</h2>
				<p class="light center header flow-text">
					Facilitating Commissions
				</p>
			</div>
			<div class="divider"></div>
			<div class="section" id="about">
				<h2 class="center header hide-on-small-only">
					About
				</h2>
				<h3 class="center header hide-on-med-and-up">
					About
				</h3>

				<h5>Blurb</h5>

				<p class="flow-text">Catalyst serves to facilitate the process of commissioning through a simple, unified, and mobile-friendly way for artists to easily list their prices, receive and track commissions, and much more.</p>

				<h5>What sets us apart</h5>

				<p class="flow-text">We are aware that there are many current and planned competitors to us. However, we plan to provide an excellent service in a way no one else does with some of the following features:</p>

				<ul class="browser-default">
					<li class="flow-text">Easy to search system</li>
					<li class="flow-text">Responsive, mobile-friendly interface</li>
					<li class="flow-text">Messages on various events such as opening of commissions</li>
					<li class="flow-text">Message delivery to E-Mail</li>
					<li class="flow-text">No social media aspect - this allows for us to focus on what matters and cut the crap</li>
					<li class="flow-text">Easy integration with other social networks</li>
					<li class="flow-text">Character management</li>
					<li class="flow-text">No fees or cuts</li>
					<li class="flow-text">And much more!</li>
				</ul>
			</div>
			<div class="divider"></div>
			<div class="section" id="roadmap">
				<h2 class="center header hide-on-small-only">
					Roadmap
				</h2>
				<h3 class="center header hide-on-med-and-up">
					Roadmap
				</h3>

				<h4>Stage One: Completed</h4>
				<h5>Initial outreach, ensure that Catalyst would be useful and that there is a demand.</h5>

				<p class="flow-text">This stage focused on the conceptualization of the project, reaching out to artists and buyers alike to see what they would want in a service such as this.  We found that there is a high demand for Catalyst, and have developed a skeleton of features for the site.</p>

				<h4>Stage Two: In progress</h4>
				<h5>Development of infrastructure and basic creation of the brand.</h5>

				<p class="flow-text">In this stage, we are developing the main components of the service.  Additionally, we are beginning to advertise and build support around the brand through community outreach, posting to social media and talking directly with fandom members.</p>

				<h4>Stage Three</h4>
				<h5>Big time advertisement, initial users.</h5>

				<p class="flow-text">During this stage, we will begin to focus on recruiting members, potentially through advertising.  We may provide incentives for artists to join and spread the word about the service.</p>

				<h4>Stage Four</h4>
				<h5>Initial user feedback and additional features.</h5>

				<p class="flow-text">Once we have a sufficient user base, we will begin to act on the feedback users provide and implement non-core features.  In this stage we will clean the interface up, polishing it.</p>

				<h4>Stage Five</h4>
				<h5>Optimization of the server and its code.</h5>

				<p class="flow-text">At this point we will be expanding exponentially, and, as such, will have increased server load.  In order to provide the best service possible to our users, we will devote this stage to optimization the site, even if it decreases load time by only a few milliseconds.</p>

				<h4>Stage Six</h4>
				<h5>???</h5>

				<p class="flow-text">By this time the service will have all of its features implemented.  In this time we will work on policy issues and bugs which may not have been seen.  Additionally, we will continue to get the feedback of our user base, potentially adding more features.</p>

				<h4>Stage Seven</h4>
				<h5>Profit</h5>

				<p class="flow-text">After the site is complete we will try to find a way to drive profit for it, either through devices such as promoted artists or donation-based services such as Patreon.</p>
			</div>
			<div class="divider"></div>
			<div class="section" id="track">
				<h2 class="center header hide-on-small-only">
					Track Development
				</h2>
				<h3 class="center header hide-on-med-and-up">
					Track Development
				</h3>

				<p class="flow-text">You can track development via <a href="https://github.com/smileytechguy/Catalyst">this</a> GitHub repository.</p>
			</div>
			<div class="divider"></div>
			<div class="section" id="involve">
				<h2 class="center header hide-on-small-only">
					Get Involved
				</h2>
				<h3 class="center header hide-on-med-and-up">
					Get Involved
				</h3>

				<p class="flow-text">There are many ways you can get involved!</p>

				<ul class="browser-default">
					<li class="flow-text">Join the <a href="#email">email list</a>, <a href="https://t.me/catalyst">Telegram group</a>, or <a href="https://discord.gg/EECUcnT">Discord server</a></li>
					<li class="flow-text">Ask me about beta testing!</li>
					<li class="flow-text">Leave feedback about Catalyst</li>
					<ul class="browser-default">
						<li class="flow-text">You may contact me through: <a href="mailto:fauxil@catalystapp.co">email</a>, <a href="https://t.me/fauxil_fox">Telegram</a>, or Discord (@Fauxil_Fox#5881)</li>
					</ul>
				</ul>
			</div>
			<div class="divider"></div>
			<div class="section" id="email">
				<h2 class="center header hide-on-small-only">
					Email List
				</h2>
				<h3 class="center header hide-on-med-and-up">
					Email List
				</h3>

				<form action="email.php" method="POST">
					<div class="row">
						<div class="input-field col s12">
							<input type="email" name="email" id="email-input" required="required">
							<label for="email-input">Email</label>
						</div>
						<div class="input-field col s12">
							<input type="text" name="context" id="context-input" required="required">
							<label for="context-input">Name and other info</label>
						</div>
						<div class="col s12">
							<button type="submit" class="btn col s12 m4 l2">Submit</button>
						</div>
					</div>
				</form>
			</div>
		</div>
		<footer class="page-footer green darken-4">
			<div class="container white-text">
				<p>
					Website &copy;2017 Fauxil Fox, All rights reserved.
				</p>
				<p>
					Hosted artwork copyright their respective owners unless otherwise stated.
				</p>
				<br>
			</div>
		</footer>
	</body>
	<!-- <script async src="https://www.googletagmanager.com/gtag/js?id=UA-REDACTED-1"></script> -->
	<script>
		// window.dataLayer = window.dataLayer || [];
		// function gtag(){dataLayer.push(arguments);}
		// gtag('js', new Date());
		// gtag('config', 'UA-REDACTED-1');

		(function($){
			$(function(){
				/* ONLOADS */
				$(".button-collapse").sideNav();
				$(".modal").modal();
				$('select').attr("required", false).material_select();
				$('.psuedo-required, .psuedo-required input').attr("required", false);
			});
		})(jQuery);
	</script>
	<script type="application/ld+json">
	{
	  "@context": "http://schema.org",
	  "@type": "Organization",
	  "url": "https://catalystapp.co",
	  "name": "Catalyst - Facilitating Commissions"
	}
	</script>
</html>
