...
<?php else: ?>
	<div class="section">
		<?php if ($_SESSION["user"]->isEmailVerificationSendable()): ?>
			<p class="flow-text">An email has been sent to <strong>
					<?= htmlspecialchars($_SESSION["user"]->getEmail()) ?>
				</strong>.</p>
		<?php else: ?>
			<p class="flow-text">An error occured in sending an email to <strong>
					<?= htmlspecialchars($_SESSION["user"]->getEmail()) ?>
				</strong>. Confirm that this email is correct. If this issue persists, contact support.</p>
		<?php endif; ?>
		...
		<p class="flow-text">Didn't receive an email?</p>
		<p>Try the following:</p>
		<blockquote>
			<p class="flow-text">Check your spam folder</p>
			<p class="flow-text">Ensure you can receive emails from <strong>
					<?= Email::NO_REPLY_EMAIL[0] ?>
				</strong></p>
			<p class="flow-text">Make sure you typed your email correctly: <strong>
					<?= htmlspecialchars($_SESSION["user"]->getEmail()) ?>
				</strong></p>
