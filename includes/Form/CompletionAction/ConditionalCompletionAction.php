...
class ConditionalCompletionAction extends AbstractCompletionAction {
	...
	public function getJs() : string {
		...
		$str = '';
		$str .= 'window.log('.json_encode(basename(__CLASS__)).', '.json_encode("checking ".count($this->getConditions())." conditions").');';
		for ($i=0; $i < count($this->getConditions()); $i++) { 
			...
			$str .= '{';
			$str .= 'window.log('.json_encode(basename(__CLASS__)).', '.json_encode("condition ".$i." (".$this->getConditions()[$i][0].") is true, invoking result").');';
		...
		if (!is_null($this->getElse())) {
			$str .= 'else';
			$str .= '{';
			$str .= 'window.log('.json_encode(basename(__CLASS__)).', '.json_encode("no conditions passed, invoking else").');';
