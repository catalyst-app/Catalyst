...
class TextField extends AbstractField {
	...
	public function getJsValidator() : string {
		...
		if ($this->getMaxLength() > 0) {
			...
			$str .= ') {';
			$str .= 'window.log('.json_encode(__CLASS__).', '.json_encode($this->getId()." - field's length is above maximum (".$this->getMaxLength().")").', true);';
		...
		$str .= ') {';
		$str .= 'window.log('.json_encode(__CLASS__).', '.json_encode($this->getId()." - field's value is in our list of disallowed values.  Naughty!").', true);';
		$str .= 'markInputInvalid('.json_encode('#'.$this->getId()).', '.json_encode($this->getErrorMessage($this->getInvalidErrorCode())).');';
		$str .= Form::CANCEL_SUBMISSION_JS;
		$str .= '}';
		...
		$str .= ') {';
		$str .= 'window.log('.json_encode(__CLASS__).', '.json_encode($this->getId()." - field's value does not pass regexp (".$this->getPattern().")").', true);';
		$str .= 'markInputInvalid('.json_encode('#'.$this->getId()).', '.json_encode($this->getErrorMessage($this->getInvalidErrorCode())).');';
		$str .= Form::CANCEL_SUBMISSION_JS;
		$str .= '}';
