	...
	public static function getFormStructure() : array {
		return [
			...
			[
				...
				"after_html" => '<p class="col s12">This will be the link to your page: <strong id="editartist-url-sample" data-base="'.(preg_replace('/Edit\/?$/', '', \Catalyst\Page\UniversalFunctions::getRequestUrl())).'">'.(preg_replace('/New\/?$/', '', \Catalyst\Page\UniversalFunctions::getRequestUrl())).(\Catalyst\User\User::isLoggedIn() ? ($_SESSION["user"]->isArtist() ? $_SESSION["user"]->getArtistPage()->getUrl() : null) : null).'</strong></p>'
			],
			[
				"name" => "desc",
				"wrapper_classes" => "col s12",
